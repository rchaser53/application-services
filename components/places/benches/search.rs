...
fn bench_search_frecent(c: &mut Criterion) {
    ...
    db_bench!(c, "search_frecent string", |db: test_db| search_frecent(
        &db,
        SearchParams {
    ...
    .unwrap());
    db_bench!(c, "search_frecent origin", |db: test_db| search_frecent(
        &db,
        SearchParams {
    ...
    .unwrap());
    db_bench!(c, "search_frecent url", |db: test_db| search_frecent(
        &db,
        SearchParams {
...
fn bench_match_url(c: &mut Criterion) {
    let test_db = TestDb::new();
    db_bench!(c, "match_url string", |db: test_db| match_url(
        &db, "mozilla"
    )
    .unwrap());
    db_bench!(c, "match_url origin", |db: test_db| match_url(
        &db,
        "blog.mozilla.org"
    )
    .unwrap());
    db_bench!(c, "match_url url", |db: test_db| match_url(
        &db,
